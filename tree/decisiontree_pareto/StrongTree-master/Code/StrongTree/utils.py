...
def get_prec(grb_model, local_data, b, beta, p, restricted_label):
    label = grb_model.label
    true_label = 0
    pred_label = 0
    ...
    for i in local_data.index:
        ...
        if yhat_i == restricted_label:
            pred_label += 1
            if y_i == restricted_label:
                true_label += 1

    if pred_label == 0:
        return -1  # or float('nan') depending on how you want to handle this case
