...
def learnTrees(depth):
    ...
    X=df[features]
    X_train, y_train = X, y
    # X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.25, stratify=y, random_state=42+run)
    ...
    # print('train num correct:', accuracy_score(y_train,train_prediction) * len(y_train))
    # print('train R2 Score:', r2_score(y_train,train_prediction))
    # print('train absolute error:', mean_absolute_error(y_train,train_prediction)*len(X_train))

    # test_prediction = dt.predict(X_test)
    # print('test accuracy:', accuracy_score(y_test,test_prediction))
    # print('test num correct:', accuracy_score(y_test,test_prediction) * len(y_test))
    # print('test R2 Score:', r2_score(y_test,test_prediction))
    # print('test absolute error:', mean_absolute_error(y_test,test_prediction)*len(X_test))
