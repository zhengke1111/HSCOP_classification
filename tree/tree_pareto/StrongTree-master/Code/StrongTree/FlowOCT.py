...
class FlowOCT:
    def __init__(self, data, label, tree, _lambda, time_limit, precision_C, restricted_label, mode):
        '''
        ...
        self.restricted_label = restricted_label
        ...
        self.model.params.Threads = 32  # in our experiments for fair comparison we set Threads = 32
        self.model.params.FeasibilityTol = 1e-09
    ...
    def create_primal_problem(self):
        ...
        # zeta[i,n,k] is the amount of flow through the edge connecting node n to sink node t_k for datapoint i
        self.zeta = self.model.addVars(self.datapoints, self.tree.Nodes + self.tree.Leaves, self.labels, vtype=GRB.BINARY, name='zeta')
        # z[i,n] is the incoming flow to node n for datapoint i
        self.z = self.model.addVars(self.datapoints, self.tree.Nodes + self.tree.Leaves, vtype=GRB.BINARY, name='z')

        # gamma: feasibility ensurance in the precision constraint
        self.gamma = self.model.addVar(vtype=GRB.CONTINUOUS, lb=0, name='gamma')
        ...
        for n in self.tree.Nodes:
            ...
            self.model.addConstrs(
                (self.z[i, n] == self.z[i, n_left] + self.z[i, n_right] + quicksum(self.zeta[i, n, k] for k in self.labels)) for i in self.datapoints)  # 11(d)
        ...
        if self.precision_C is not None:
                # choose which label is treated as "positive"
                if self.restricted_label is not None:
                    pos_label = self.restricted_label
                else:
                    # default: use label 1 if present, otherwise the first label
                    pos_label = 1 if 1 in list(self.labels) else list(self.labels)[0]

                # LHS: true positives = sum_{i: y_i = pos} sum_n zeta[i,n,pos]
                lhs = quicksum(
                    self.zeta[i, n, pos_label]
                    for i in self.datapoints
                    if self.data.at[i, self.label] == pos_label
                    for n in (self.tree.Nodes + self.tree.Leaves)
                )  

                # RHS: C * predicted positives = C * sum_{i} sum_n zeta[i,n,pos]
                rhs = self.precision_C * quicksum(
                    self.zeta[i, n, pos_label]
                    for i in self.datapoints
                    for n in (self.tree.Nodes + self.tree.Leaves)
                )

                self.model.addConstr(lhs + self.gamma >= rhs, name="precision_constraint")
                self.model.addConstr(lhs + self.gamma >= 1, name="precision_denominator_not_0")

        self.model.addConstrs((self.z[i,1] == 1) for i in self.datapoints)  # (11f)
        ...
        if self.mode == "classification":  # zeta[i,n] <= beta[n,y[i]]     forall n in N+L, i
            ...
            for n in self.tree.Nodes + self.tree.Leaves:
                for i in self.datapoints:
                    self.model.addConstrs(
                        self.zeta[i, n, k] <= self.beta[n, k] for k in self.labels)  # 11(i)
        ...
        for n in self.tree.Leaves:
            self.model.addConstrs(quicksum(self.zeta[i, n, k] for k in self.labels) == self.z[i, n] for i in self.datapoints)  #11(e)

        # define objective function
        obj = LinExpr(0)
        for i in self.datapoints:
            obj.add((1 - self._lambda) * (quicksum(self.zeta[i, n, self.data.at[i, self.label]] for n in self.tree.Nodes + self.tree.Leaves) - self.m[i]))  # 11(a)

        for n in self.tree.Nodes:
            for f in self.cat_features:
                obj.add(-1 * self._lambda * self.b[n, f])

        obj.add(-1 * 1e6 * self.gamma)
