...
def calculate_gamma(X_train, y_train, weights, bias, beta_p, epsilon):
    ...
    p = X_train.shape[1]
    sum_tp = sum(heaviside_closed(0, sum(weights[i]*X_train[s][i] for i in range(p)) + bias) for s in positive_index)  
    sum_dm = sum(heaviside_closed(0, sum(weights[i]*X_train[s][i] for i in range(p)) + bias) for s in positive_index) + sum(heaviside_open(-epsilon, sum(weights[i]*X_train[s][i] for i in range(p)) + bias) for s in negative_index)
    gamma = max(0, - (sum_tp - beta_p * sum_dm))
    z_plus_0_start, z_plus_start, z_minus_start = {}, {}, {}

    for s in positive_index:
        z_plus_0_start[s] = heaviside_closed(0, sum(weights[i]*X_train[s][i] for i in range(p)) + bias - 1)
        z_plus_start[s] = heaviside_closed(0, sum(weights[i]*X_train[s][i] for i in range(p)) + bias)
    for s in negative_index:
        z_plus_0_start[s] = heaviside_closed(0, -sum(weights[i]*X_train[s][i] for i in range(p)) - bias - 1)
        z_minus_start[s] = heaviside_closed(0, -(sum(weights[i]*X_train[s][i] for i in range(p)) + bias) - epsilon)
...
def calculate_z_plus_0(X_train, y_train, weights, bias):
    """
...
def calculate_delta(X_train, y_train, weight, bias, epsilon, ratio):
    ...
    if epsilon is not None:
        ...
        for s in positive_index:
            # \phi_{0s}^+
            if sum(weight[i]*X_train[s][i] for i in range(p)) + bias - 1 > 0:
                value_ge[0].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias - 1)
            elif sum(weight[i]*X_train[s][i] for i in range(p)) + bias - 1 < 0:
                value_le[0].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias - 1)
            else:
                if index_odd == 0:
                    value_ge[0].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias - 1)
                else:
                ...
                index_odd += 1
            # \phi_{1s}^+
            if sum(weight[i]*X_train[s][i] for i in range(p)) + bias > 0:
                value_ge[1].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias)
            elif sum(weight[i]*X_train[s][i] for i in range(p)) + bias < 0:
                value_le[1].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias)
            else:
                if index_odd == 0:
                    value_ge[1].append(sum(weight[i]*X_train[s][i] for i in range(p)) + bias)
                else:
        ...
        for s in negative_index:
            ...
            if -sum(weight[i]*X_train[s][i] for i in range(p)) - bias - 1 > 0:
                ...
                index_odd += 1
            # \phi_{1s}^-
            if -(sum(weight[i]*X_train[s][i] for i in range(p)) + bias) - epsilon > 0:
                value_ge[1].append(-(sum(weight[i]*X_train[s][i] for i in range(p)) + bias) - epsilon)
            elif -(sum(weight[i]*X_train[s][i] for i in range(p)) + bias) - epsilon < 0:
                value_le[1].append(-(sum(weight[i]*X_train[s][i] for i in range(p)) + bias) - epsilon)
            else:
                if index_odd == 0:
                    value_ge[1].append(-(sum(weight[i]*X_train[s][i] for i in range(p)) + bias) - epsilon)
                else:
        ...
        if ratio < 100:
            delta_1[0] = max(1e-5, np.percentile(value_ge[0], ratio))
            delta_2[0] = max(1e-5, -np.percentile(value_le[0], 100 - ratio))
        else:
...
def evaluate_binary(X, y, weight, bias):  # X, y, w, b obtained by PIP
    ...
    p = X.shape[1]
    scores = np.array([sum(X[s][i] * weight[i] for i in range(p)) + bias for s in range(X.shape[0])])
    predictions = np.zeros_like(y)  # Initialize predictions array

    scores_out_of_margin = y*scores-1
...
def train_lr_model(X_train, y_train, n_iters=100):
    ...
    w_array = clf.coef_[0]
    w_dict = {i: w_array[i] for i in range(X_train.shape[1])}

    result = {'w': w_dict, 'b': clf.intercept_[0]}
    return result


def write_single_integrated_result(results_csv, dataset, split, method, beta, objective_value, optimality_gap, time, actual_time, gamma, train_acc_margin, test_acc_margin, 
                                   train_acc, test_acc, train_prec, test_prec, train_recall, test_recall):
    with open(results_csv, mode='a', newline='') as f:
        ...
        writer.writerow([
            dataset, split, method, beta, objective_value, optimality_gap, time, actual_time, gamma, 
            train_acc_margin, test_acc_margin, train_acc, test_acc, train_prec, test_prec, train_recall, test_recall
        ])
