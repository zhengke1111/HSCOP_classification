...
def solve_binary_classification_prob(param, dataset_results_csv, dataset_dir):
    ...
    def run_algorithm(method, X_train, y_train, dataset, beta, epsilon, model_params, result_dir, w_start, b_start, save_log=False, console_log=False):
        ...
        elif method == "Early MIP":
            mip = Model(X=X_train, y=y_train, epsilon=epsilon, beta=beta, model_type='full_early_termination', delta_plus=None, delta_minus=None, model_params=model_params, model_dir=result_dir,
                        model_name='FullModel', save_log=save_log, console_log=console_log)
            mip.formulate_model(w_start, b_start)
            mip.solve_model()
            return mip
            ...
            if state is True:
                ...
                if method != 'U-PIP':
                    for beta in THRESHOLD_GRID[param['dataset']]:
                        ...
                        if method == 'Full MIP' or 'Early MIP':
...
def run_binary_experiment(method_dict, pareto = False):
    dataset_list_ = DATASET_LIST

    for dataset in dataset_list_:
        dataset_dir = f"binary/results/{dataset}"
        ...
        param = {
            ...
            'threshold': THRESHOLD_GRID[dataset] if pareto == False else THRESHOLD_GRID_PARETO[dataset],
            'save_log': True,
            'console_log': True
        }
        solve_binary_classification_prob(param, dataset_results_csv, dataset_dir)

method_dict = {'Full MIP': False, 'Early MIP': False, 'Fixed PIP': False, 'Shrinkage PIP':False, 'U-PIP': False}
run_binary_experiment(method_dict, pareto = False)