...
dataset = 'ctmc_5buckets'
current_datetime = datetime.now()
result_dir = os.getcwd() + '/decisiontree_pareto/StrongTree-master/Results'
...
with open(result_csv, mode='a', newline='') as all_result:
    ...
    writer.writerow(['approach', 'dataset', 'sample', 'nrow', 'depth', 'restricted_class', 'beta_p', 'lambda', 
...
#         # FlowOCTReplication.main(["-f", dataset+'.csv', "-o", result_csv, "-d", 2, "-t", 3600, "-l", 0, "-i", sample, "-c", 1, "-p", 0.75, "-r", 0])
#             BendersOCTReplication.main(["-f", dataset +'.csv', "-o", result_csv, "-d", depth, "-t", 3600, "-l", _lambda, "-i", sample, "-c", 1, "-r", 0])


def tune_lambda_from_validation(validation_csv_path: str, seed: int | None = None):
    """
...
tuned_lambda_map = tune_lambda_from_validation(validation_csv, seed=None)
