...
def main(argv):
    ...
    random_states_list = [43, 44, 45, 46]

    try:
        opts, args = getopt.getopt(argv, "f:o:d:t:l:i:c:p:r:m",
                                   ["input_file=", "output_file=", "depth=", "timelimit=", "lambda=",
                                    "input_sample=",
                                    "calibration=", "precision_C=", "restricted_label=", "mode="])
    ...
    for opt, arg in opts:
        if opt in ("-f", "--input_file"):
            ...
            calibration = int(arg)
        elif opt in ("-p", "--precision_C"):
            precision_C = float(arg)
        elif opt in ("-r", "--restricted_label"):
            restricted_label = int(arg)
        elif opt in ("-m", "--mode"):
            mode = arg

    start_time = time.time()
    data_path = os.getcwd() + '/decisiontree_pareto/StrongTree-master/DataSets/'
    ...
    out_put_path = os.getcwd() + '/decisiontree_pareto/StrongTree-master/Results/'
    ...
    '''
    data_train, data_test = train_test_split(data, test_size=0.25, stratify=data['target'], random_state=random_states_list[input_sample - 1])
    data_train_calibration, data_calibration = train_test_split(data_train, test_size=1/3, stratify = data_train['target'],
    ...
    primal = FlowOCT(data_train, label, tree, _lambda, time_limit, precision_C, restricted_label, mode)
    ...
    train_prec = test_prec = calibration_prec = 0
    ...
    if mode == "classification":
        ...
        train_prec = get_prec(primal, data_train, b_value, beta_value, p_value, restricted_label)
        test_prec = get_prec(primal, data_test, b_value, beta_value, p_value, restricted_label)
        calibration_prec = get_prec(primal, data_calibration, b_value, beta_value, p_value, restricted_label)
    ...
    #     print("train prec", train_prec)
    #     print("test prec", test_prec)
    #     print("calibration prec", calibration_prec)
    # elif mode == "regression":
