...
class precisionbooleanoptimalDecisionTreeClassifier:
    """
    optimal classification tree
    """
    def __init__(self, max_depth=3, min_samples_split=1, alpha=0.01,N=5, warmstart=True, timelimit=600, restricted_class = 1, precision_threshold = 0.80, output=True):
    ...
    def fit(self, x, y):
        ...
        m, a, b, c, d, l, gamma = self._buildMIP(x, y)
        if self.warmstart:
    ...
    def _buildMIP(self, x, y):
        ...
        """
        N_total = self.n
        N_restricted = int(np.sum(y == self.restricted_class))
        ...
        # ============= Use the same numeric precision as PIP =============================
        m.params.MIPFocus = 1
        m.params.IntegralityFocus = 1
        m.params.NumericFocus = 3
        m.params.Threads = 32  # in our experiments for fair comparison we set Threads = 32
        ...
        # ================= precision constraint =====================================================
        eta = m.addVars(self.l_index, vtype=GRB.CONTINUOUS, name='eta') 
        zeta = m.addVars(self.l_index, vtype=GRB.CONTINUOUS, name='zeta') 
        gamma = m.addVar(lb=0, vtype=GRB.CONTINUOUS, name='gamma') 
        # ============================================================================================
        ...
        # objective function
        error=L.sum() / self.n 
        obj = error + self.alpha * gp.quicksum(a[j,t] for j in range(self.p) for t in self.b_index) + 1e4 * gamma
        m.setObjective(obj)
        ...
        # ================= precision constraint =====================================================
        m.addConstrs(eta[t] <= N_restricted * c[self.restricted_class,t] for t in self.l_index)
        m.addConstrs(eta[t] <= gp.quicksum((y[i] == self.restricted_class) * z[i,t] for i in range(self.n)) + N_restricted * (1-c[self.restricted_class,t]) for t in self.l_index)

        m.addConstrs(zeta[t] >= -N_total * c[self.restricted_class,t] for t in self.l_index)
        m.addConstrs(zeta[t] >= z.sum('*', t) - N_total * (1- c[self.restricted_class,t]) for t in self.l_index)

        m.addConstr(gp.quicksum(eta[t] for t in self.l_index) - self.precision_threshold* gp.quicksum(zeta[t] for t in self.l_index) + gamma >= 0)
        m.addConstr(gp.quicksum(eta[t] for t in self.l_index) + gamma >= 1)
        # =============================================================================================    
       
        
        

        return m, a, b, c, d, l, gamma
    ...
    def _setStart(self, x, y, a, c, d, l):
        ...
        if self.min_samples_split > 1:
            clf = tree.DecisionTreeClassifier(max_depth=self.max_depth, min_samples_split=self.min_samples_split, random_state = 42)
        else:
