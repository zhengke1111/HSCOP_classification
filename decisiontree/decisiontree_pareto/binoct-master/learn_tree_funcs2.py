...
def sget_bounds(tree, index):
    ...
    if tree.tree_.children_left[left] != -1:
        leftlb,leftub = sget_bounds(tree, left)
    else:
        leftlb,leftub = sget_leaf_constant(tree,left),sget_leaf_constant(tree,left)
        
    if tree.tree_.children_right[left] != -1:
        rightlb,rightub = sget_bounds(tree, right)
    else:
        rightlb,rightub = sget_leaf_constant(tree,right), sget_leaf_constant(tree,right)
