    ...
    for seed in seeds: 
        for alpha in [0.001, 0.01]:
            for N in [3, 5]:
                ...
                brtree = miptree.precisionbooleanoptimalDecisionTreeClassifier(max_depth=depth, min_samples_split=1, alpha=alpha, N=N, warmstart=True, timelimit=time_limit, restricted_class= restricted_label, precision_threshold= beta_p, output=True)
                ...
                with open(result_csv, mode='a') as results:
                    ...
                    results_writer.writerow(
                        [approach_name, dataset, seed, train_len, depth, restricted_label, beta_p, alpha, N, 
                            time_limit, brtree.status, brtree.objval, brtree._gamma, brtree.runtime, brtree.optgap, 
